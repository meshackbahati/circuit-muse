...
# miss every time). With the persistent dir, /var/lib/circuit-muse-build/<target>/
...
#
# Set CIRCUIT_MUSE_PERSISTENT_BUILD_DIR=0 to fall back to the legacy tempfile flow
# without rebuilding the image (escape hatch if the persistent dir misbehaves
# in production).
_BUILD_ROOT = Path(os.environ.get('CIRCUIT_MUSE_BUILD_ROOT', '/var/lib/circuit-muse-build'))
_USE_PERSISTENT_DIR = (
    os.environ.get('CIRCUIT_MUSE_PERSISTENT_BUILD_DIR', '1')
...
class ESPIDFCompiler:
    ...
    def _find_arduino_libraries_dir(self) -> Path | None:
        ...
        P2.1h: when the pro overlay sets CIRCUIT_MUSE_FALLBACK_LIBRARIES_DIR (the
        ...
        _fb = os.environ.get('CIRCUIT_MUSE_FALLBACK_LIBRARIES_DIR')
        if _fb:
    ...
    def _resolve_library_components(
        ...
        cmake_content = (
            '# Auto-generated by CircuitMuse — all user libraries merged into one component.\n'
    ...
    def _create_idf_component(
        ...
        cmake_content = (
            f'# Auto-generated by CircuitMuse for library: {lib_dir_name}\n'
    ...
    _PARTITION_CSVS: dict[str, str] = {
        # CircuitMuse's historical default: single huge factory app, no OTA.
    ...
    _DEFAULT_OPTIONS: dict[str, str | int | bool] = {
        'partitionScheme': 'huge_app',  # historical CircuitMuse default
        'cpuFreqMHz': 240,
        'flashMode': 'dio',
        'flashSize': '4MB',
        'flashFreqMHz': '40',
    ...
    def _merge_flash_image(
        ...
        app = build_dir / 'circuit-muse-sketch.bin'
    ...
    async def compile(
        ...
        Build dir layout:
        - With CIRCUIT_MUSE_PERSISTENT_BUILD_DIR=1 (default): a per-target dir at
          /var/lib/circuit-muse-build/<idf_target>/project/ is reused across compiles.
          ninja's incremental cache + ccache hits combine to bring warm
          compiles down to ~5-30s.
        - With CIRCUIT_MUSE_PERSISTENT_BUILD_DIR=0: legacy tempfile.TemporaryDirectory
    ...
    async def _compile_in_dir(
        ...
        # QEMU's WiFi AP broadcasts "CircuitMuse-GUEST" on channel 6.
        # We normalize ANY user SSID → "CircuitMuse-GUEST", enforce channel 6,
        ...
        if self.has_arduino:
            ...
            # Prepend Arduino.h + circuit-muse_compat.h if not already included.
            # circuit-muse_compat.h shims arduino-esp32 3.x APIs (ledcAttach, …)
            # onto the 2.0.17 toolchain we currently pin. See
            # esp-idf-template/main/circuit-muse_compat.h.
            if '#include' not in main_content or 'Arduino.h' not in main_content:
                main_content = (
                    '#include "Arduino.h"\n'
                    '#include "circuit-muse_compat.h"\n' + main_content
                )
            else:
