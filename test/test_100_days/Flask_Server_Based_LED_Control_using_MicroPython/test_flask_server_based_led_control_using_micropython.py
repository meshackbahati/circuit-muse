"""
Velxio emulation test for: Flask_Server_Based_LED_Control_using_MicroPython

Board:    esp32
Features: wifi, flask, http_server
Languages: py

Notes / known limitations:
#   * WiFi via QEMU slirp NIC (-nic user,model=esp32_wifi).
#   * Host-side companion code present (flask) — that half runs on the host, not in Velxio.
...
PROJECT_NAME = 'Flask_Server_Based_LED_Control_using_MicroPython'
PROJECT_BOARD_KIND: BoardKind = 'esp32'
EXPECTED_FEATURES = ['wifi', 'flask', 'http_server']


class Test_Flask_Server_Based_LED_Control_using_MicroPython_StaticAnalysis(unittest.TestCase):
...
class Test_Flask_Server_Based_LED_Control_using_MicroPython_LiveBackend(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_backend_websocket_handshake(self):
        ...
        url = backend_websocket_url("test-100-days-flask_server_based_led_control_using_micropython")
