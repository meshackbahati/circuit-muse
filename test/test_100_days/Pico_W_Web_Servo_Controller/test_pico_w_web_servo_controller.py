"""
Velxio emulation test for: Pico_W_Web_Servo_Controller

Board:    pico-w
Features: wifi, servo, ota
Languages: py

Notes / known limitations:
#   * WiFi via QEMU slirp NIC (-nic user,model=esp32_wifi).
#   * OTA update path is NOT asserted (no real flash partition table).
...
PROJECT_NAME = 'Pico_W_Web_Servo_Controller'
PROJECT_BOARD_KIND: BoardKind = 'pico-w'
EXPECTED_FEATURES = ['wifi', 'servo', 'ota']


class Test_Pico_W_Web_Servo_Controller_StaticAnalysis(unittest.TestCase):
...
class Test_Pico_W_Web_Servo_Controller_LiveBackend(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_backend_websocket_handshake(self):
        ...
        url = backend_websocket_url("test-100-days-pico_w_web_servo_controller")
