"""
Velxio emulation test for: Smart_IoT_Gas_Monitoring_System

Board:    esp32
Features: wifi, flask, matplotlib, dht, http_server
Languages: py

Notes / known limitations:
#   * WiFi via QEMU slirp NIC (-nic user,model=esp32_wifi).
#   * Host-side companion code present (flask, matplotlib) — that half runs on the host, not in Velxio.
...
PROJECT_NAME = 'Smart_IoT_Gas_Monitoring_System'
PROJECT_BOARD_KIND: BoardKind = 'esp32'
EXPECTED_FEATURES = ['wifi', 'flask', 'matplotlib', 'dht', 'http_server']


class Test_Smart_IoT_Gas_Monitoring_System_StaticAnalysis(unittest.TestCase):
...
class Test_Smart_IoT_Gas_Monitoring_System_LiveBackend(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_backend_websocket_handshake(self):
        ...
        url = backend_websocket_url("test-100-days-smart_iot_gas_monitoring_system")
